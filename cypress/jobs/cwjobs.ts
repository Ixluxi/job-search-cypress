...
import CwjobsPage from '../support/page-object-model/cwjobs-page'
const cwjobsPage = new CwjobsPage()
...
it('CW jobs', () => {
    ...
    cwjobsPage.acceptCookiesButton.click()
    cwjobsPage.firstSignInButton.click()
    cwjobsPage.secondSignInButton.click()
    cwjobsPage.emailAddressTextBox.type(Cypress.env('cwjobsUsername'))
    cwjobsPage.passwordTextBox.type(Cypress.env('cwjobsPassword'))
    cwjobsPage.loginButton.click()
    cy.wait(generateRandomNumber())
    ...
    for (const searchTerm of searchTerms) {
        ...
        cy.task('getSetFileContents', { fileAction: FileActions.GET, fileName: 'investigate' }).then(investigate => {
            cy.task('getSetFileContents', { fileAction: FileActions.GET, fileName: 'ignore' }).then(ignore => {
                ...
                cy.visit(`${Cypress.env('cwJobsBaseUrl')}/jobs/permanent/${searchTerm}?salary=${salaryFrom}&salarytypeid=1&action=facet_selected%3bsalary%3b${salaryFrom}%3bsalarytypeid%3b1&postedWithin=${daysPostedWithin}`)

    ...
    for (const searchTerm of searchTerms) {
        ...
        cy.task('getFileContents', 'manualApplications').then(manualApplications => {
            cy.task('getFileContents', 'ignore').then(ignoreUrls => {
                ...
                cy.visit(`${Cypress.env('totalJobsUrl')}/jobs/permanent/${searchTerm}?salary=30000&salarytypeid=1&sort=3&action=facet_selected%3bage%3b3&postedWithin=3`, { timeout: 120000 }).then(() => {
